$(function () {
    const $container = $('.proto-container_stage');

    var index = $container.find('.row-colonne_stage').length;


    $(document).on('select2:select', '.select-type', function (e) {
        const $this = $(this);
        let field_str = $this.find('option:selected').attr('data-require-fields');
        const $parent = $this.closest('.row-colonne_stage');
        let fields = [];
    ...
    if (index > 0) {
        $container.children('.row-colonne_stage').each(function() {
            const $this = $(this);
            addDeleteLink($this);
    ...
    function addDeleteLink($prototype, name = null) {
        ...
        // Ajout du lien
        $prototype.find(".del-col_stage").append($deleteLink);
